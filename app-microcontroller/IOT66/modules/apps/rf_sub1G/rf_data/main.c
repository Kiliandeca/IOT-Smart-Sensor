...
void handle_rf_rx_data(void)
{
    ...
    {
        // Récupération du contenu du message, et copie dans le buffer data
        memcpy(&msg_data, &data[4], sizeof(message)); 

        int8_t data_to_decrypt[3] = {msg_data.temp, msg_data.hum, msg_data.lum};

        decrypt(data_to_decrypt, key);

        
        msg_data.temp = data_to_decrypt[0];
        msg_data.hum = data_to_decrypt[1];
        msg_data.lum = data_to_decrypt[2];
    }
}
/***************************************************************************** */
...
void send_on_rf(void)
{
    ...
    // Traitement des données à envoyer : on décompose les valeurs en centaine / dizaine / unité car le buffer de chiffrement est sur 8 bits (jusqu'à 128)
    uint8_t data_to_send[8] = {cc_tx_msg.lum / 100, (cc_tx_msg.lum - (cc_tx_msg.lum / 100) * 100) / 10, cc_tx_msg.lum % 10, cc_tx_msg.temp / 100, (cc_tx_msg.temp - (cc_tx_msg.temp / 100) * 100) / 10, cc_tx_msg.temp % 10, (cc_tx_msg.hum / 10), (cc_tx_msg.hum % 10)};

    // Chiffrement de la trame avant envoi
    encrypt(data_to_send, key);

    // Construction de la trame, on copie les données dans un buffer
    uint8_t cc_tx_data[12];
    cc_tx_data[0] = 11;   
    cc_tx_data[1] = NEIGHBOR_ADDRESS;
    cc_tx_data[2] = DEVICE_ADDRESS;
    cc_tx_data[3] = cc_tx_data[0] + cc_tx_data[1] + cc_tx_data[2];
    cc_tx_data[4] = data_to_send[0];
    cc_tx_data[5] = data_to_send[1];
    cc_tx_data[6] = data_to_send[2];
    cc_tx_data[7] = data_to_send[3];
    cc_tx_data[8] = data_to_send[4];
    cc_tx_data[9] = data_to_send[5];
    cc_tx_data[10] = data_to_send[6];
    cc_tx_data[11] = data_to_send[7];
...
{
    // Si le décalage d'entrée est supérieur à la taille de la chaîne, on rectifie la valeur de décalage
    if (shiftBy > size)
    {
        shiftBy = shiftBy - size;
    }

    // Si la taille de la chaine est 1, on ne fait rien
    if (size == 1)
    ...
    else
    {
        ...
        // On lit les caractère un a un, on les sauvegarde, puis on les copie au nouvel index
        for (int i = 0; i < size - shiftBy; i++)
        {                                      
            temp = myarray[i];                
            myarray[i] = myarray[i + shiftBy]; 
            myarray[i + shiftBy] = temp;       
        }
    }
}
...
void gestionAffichage(message info)
{
	...
	*/
    ...
    { // Exemple : température en premier, luminosité en second et humidité en troisième
        ligne_temp = 3;
        ligne_lum = 4;
        ligne_hum = 5;
    }
    ...
    {                                             // si toutes les lignes (axe y) des données ont été définit

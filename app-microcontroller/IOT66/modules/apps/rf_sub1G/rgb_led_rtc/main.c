...
static volatile uint32_t cc_tx2 = 0;
static volatile uint8_t cc_tx_buff2[RF_BUFF_LEN2];
static volatile uint8_t cc_ptr2 = 0;
void handle_uart_cmd(uint8_t c)
{
	uprintf(UART0, "Caractere : %c",c);
	if (cc_ptr2 < RF_BUFF_LEN2) {
		cc_tx_buff[cc_ptr2++] = c;
	} else {
		cc_ptr2 = 0;
	}
	if ((c == '\n') || (c == '\r')) {

...
void dump_record(int offset)
{
	...
	if (strncmp((char*)(mmc_data + offset), "Pump ", 5) == 0) {
		printf("\t- %s\n", mmc_data + offset);
		printf("\t- CSoil: %d\n", (uint16_t)(*(mmc_data + offset + 16)));
		return;
	}

	data = (uint16_t*)(mmc_data + offset);
	printf("\t- Int temp: %d,%02d degC\n", data[0] / 10,  (data[0]> 0) ? (data[0] % 10) : ((-data[0]) % 10));
	printf("\t- Vbat: %d.%02d V\n", data[1]/1000, (data[1]/10)%100);
	printf("\t- CSoil: %d\n", data[2]);
	printf("\t- CTemp: %d,%d°C\n", data[3]/10, (data[3]> 0) ? (data[3] % 10) : ((-data[3]) % 10));
	offset += 8;

	/* Print exanh sensor record data */
	data = (uint16_t*)(mmc_data + offset);
	if (data[0] != 0) {
...
int dump_block(int source_fd, int dump_invalid)
{
	int offset = 0;
	struct rtc_time date;
	int32_t tmp101_deci_degrees = 0, abs_deci = 0;
	uint32_t mv_batt = 0;
	...
	/* Block writing date */
	memcpy(&date, (mmc_data + offset), sizeof(struct rtc_time));
	date_dump(&date, "Block writting date");
	/* Begining of data */
	offset = 64;
...
int check_dump_valid(int source_fd)
{
	...
	if (strncmp((char*)mmc_data, MMC_BLK_VALID_IDX_MAGIC, MMC_BLK_VALID_IDX_MAGIC_SIZE) != 0) {
		printf("Error : block 1 does not hold a valid magic\n");
		return -1;
	} else {
		static uint32_t maybe_last_valid_block = 0;
		printf("Valid first block found.\n");

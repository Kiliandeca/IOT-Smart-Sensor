...
uint16_t crc_ccitt(uint16_t crc, uint8_t* buf, int len)
{
	...
	for (i = 0; i < len; i++) {
		crc = (crc << 8) ^ crc_ccitt_table[ ((crc >> 8) ^ (uint16_t)buf[i]) & 0x00FF ];
	}
	return crc;
}
...
/***************************************************************************** */
/* The following program got used to generate the above table : */
#if 0
#include <stdio.h>
#include <stdint.h>
...
int main(void)
{
    ...
    printf("static const uint16_t crc_ccitt_table[256] = {");

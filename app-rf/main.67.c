...
void handle_rf_rx_data(void)
{
	...
	if(data[2] == 66 && data[1] == 67){
		...
		uprintf(UART0, "Mauvaise source ou destination \n\r");
...
void send_on_rf(void)
{
	...
	cc_tx_data[3] = cc_tx_data[0] + cc_tx_data[1] + cc_tx_data[2];				// + cc_tx_msg.hum + cc_tx_msg.lum + cc_tx_msg.temp
...
int main(void)
{
	...
	while (1) {
		...
		uprintf(UART0, "CHECK : %d", check_rx);
		if (check_rx == 1) {

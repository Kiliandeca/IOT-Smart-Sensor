...
#define DEVICE_ADDRESS 66 /* Addresses 0x00 and 0xFF are broadcast */
#define NEIGHBOR_ADDRESS 67 /* Address of the associated device */
/***************************************************************************** */
/* Pins configuration */
...
void handle_rf_rx_data(void)
{
	...
	int crc = data[0] + data[1] + data[2];
	
	// MODIFICATION
	uprintf(UART0, "============== MSG RECU ================ \n\r");
	uprintf(UART0, "DATA [0]  :  %d \n\r", data[0]);
	uprintf(UART0, "DATA [1]  :  %d \n\r", data[1]);
	uprintf(UART0, "DATA [2]  :  %d \n\r", data[2]);
	uprintf(UART0, "msg_data.lum :  %d \n\r", msg_data.lum);
	uprintf(UART0, "CRC CALCULE :  %d \n\r", crc);
	uprintf(UART0, "============== FIN DE LA RECEPTION ================ \n\r");
...
void send_on_rf(void)
{
	...
	cc_tx_data[0]=sizeof(message)+3;  	//length
	cc_tx_data[1]=DEVICE_ADDRESS;		//source
	cc_tx_data[2]=NEIGHBOR_ADDRESS;		//dest
	cc_tx_data[3] = cc_tx_data[0] + cc_tx_data[1] + cc_tx_data[2]; // CRC
	...
	if (cc1101_tx_fifo_state() != 0) {
		uprintf(UART0, "TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT\n\r" );
    ...
    uprintf(UART0, "\t RF: DESTI?NATOIN:  %d \n\r", cc_tx_data[2]);
	uprintf(UART0, "\t RF: CRC : %d \n\r", cc_tx_data[3]);
    uprintf(UART0, "\t RF: DATA : %d \n\r", cc_tx_data[4]);
	uprintf(UART0, "\t RF: TEMPERATURE : %d \n\r", cc_tx_msg.temp);
	uprintf(UART0, "#- FIN MESSAGE ENVOYE : \n\r" );
#endif
}
...
int main(void)
{
	...
	/* Configure and start display SCREEN  */
	ret = ssd130x_display_on(&display);
	/* Erase screen with lines, makes it easier to know things are going right SCREEN */
	...
	while (1) {
		...
		if (update_display == 1) {
			...
			snprintf(data, 20, "Temp: %d.%02d", temp);
